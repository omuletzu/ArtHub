...
class settings_block_users : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_settings_block_users)
        ...
        val user_ref = FirebaseFirestore.getInstance().collection("users").document(Firebase.auth.currentUser?.uid.toString())
            .collection("blocked_users").document("blocked_users")
            ...
            .addOnSuccessListener { task ->
                ...
                if(task.exists()) {
                    ...
                    add_user.setOnClickListener(){
                        ...
                        FirebaseFirestore.getInstance().collection("users").whereEqualTo("nickname", user.text.toString().trim().lowercase()).get()
                            .addOnSuccessListener { task_verify ->
                                if(!task_verify.isEmpty && !str_arr.contains(user.text.toString().trim().lowercase())){

                                    str_arr.add(user.text.toString().trim().lowercase())
                                    recycle.adapter = add_recycler_adapter(str_arr, recycle, user_ref, 0)
                                    ...
                                    user_ref.update(
                                        mapOf(
                                            ...
                                            user.text.toString().trim().lowercase() to task_verify.documents[0].id
                                        )
                                    )
